...
// ListIncomingPayment implements [invoice_ifaceconnect.InvoiceServiceHandler]. It
// lists the INCOMING payments of the scoped team (for_team_id = receiver),
// optionally filtered by payer and status.
func (s *invoiceServiceImpl) ListIncomingPayment(
	ctx context.Context,
	req *connect.Request[invoice_iface.ListIncomingPaymentRequest],
) (*connect.Response[invoice_iface.ListIncomingPaymentResponse], error) {
	pay := req.Msg
	if pay.Page == nil {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("page is required"))
	}

	result := &invoice_iface.ListIncomingPaymentResponse{
